...
- Header와 Footer를 각각의 독립된 html 파일로 나누어 세부 페이지 제작시 페이지에 공통으로 포함되는 Section을 추가 코드 작성 없이 재사용할 수 있도록 설계하였습니다.
